  ...
  return (
    <Card title={t("sales.retailCardTitle")} subtitle={t("sales.retailCardSub")}>
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label={t("sales.customerName")}><input className="input" placeholder="e.g. S. Hassan" /></Field>
        <Field label={t("sales.itemDesc")}><input className="input" placeholder="e.g. 22K Ring" defaultValue="Classic Band Ring" /></Field>
        <Field label={t("sales.weight")}><input type="number" step="0.1" value={weight} onChange={(e) => setWeight(+e.target.value)} className="input" /></Field>
        ...
        <Field label={t("sales.laborCost")}><input type="number" value={labor} onChange={(e) => setLabor(+e.target.value)} className="input" /></Field>
        <Field label={t("sales.profitMargin")}><input type="number" value={profit} onChange={(e) => setProfit(+e.target.value)} className="input" /></Field>
      </div>
      ...
      <button className="mt-6 inline-flex items-center gap-2 rounded-lg bg-gradient-gold px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-luxury">
        <Receipt className="h-4 w-4" /> {t("sales.confirmRetail")}
      </button>
    </Card>
  );
}
...
function InvestmentForm({ price }: { price: number }) {
  ...
  return (
    <Card title={t("sales.investCardTitle")} subtitle={t("sales.investCardSub")}>
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label={t("sales.clientEntity")}><input className="input" placeholder="e.g. Al Noor Holdings" /></Field>
        <Field label={t("sales.certificateId")}><input className="input" placeholder="Auto: CRT-00234" defaultValue="CRT-00234" /></Field>
        <Field label={t("sales.weight")}><input type="number" step="1" value={weight} onChange={(e) => setWeight(+e.target.value)} className="input" /></Field>
        <Field label={t("sales.purity")}>
          <select value={karat} onChange={(e) => setKarat(+e.target.value)} className="input">
            {[24, 22, 21].map((k) => <option key={k} value={k}>{k}K</option>)}
          </select>
        </Field>
        <Field label={t("sales.premiumSpot")}><input type="number" step="0.1" value={premium} onChange={(e) => setPremium(+e.target.value)} className="input" /></Field>
        <Field label={t("sales.delivery")}>
          <select className="input">
            <option>{t("sales.vaultStorage")}</option>
            <option>{t("sales.physicalPickup")}</option>
            <option>{t("sales.insuredCourier")}</option>
          </select>
        </Field>
      </div>
      <div className="mt-5 rounded-xl border border-primary/30 bg-gradient-to-br from-accent to-card p-4">
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <ValueLine label={`${t("sales.spotValue")} (${premium}%)`} value={fmt(premiumValue)} />
          <ValueLine label={t("sales.totalVat")} value={fmt(total)} highlight />
        </div>
        <div className="mt-2 text-[11px] text-muted-foreground">{t("sales.formulaInvest")} {fmt(price)}{t("sales.formulaInvestSuffix")}</div>
      </div>
      <button className="mt-6 inline-flex items-center gap-2 rounded-lg bg-gradient-gold px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-luxury">
        <FileCheck2 className="h-4 w-4" /> {t("sales.issueCert")}
      </button>
    </Card>
  );
}
  ...
  return (
    <div className="mt-5 rounded-xl border border-primary/30 bg-gradient-to-br from-accent to-card p-4">
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <ValueLine label={t("sales.vat")} value={fmt(vat)} />
        <ValueLine label={t("sales.total")} value={fmt(total)} highlight />
      </div>
      <div className="mt-2 text-[11px] text-muted-foreground">{t("sales.formulaRetail")} {fmt(price)}{t("sales.formulaRetailSuffix")}</div>
    </div>
  );
}
